...
#     predictor_sizes = [model.get_layer('classes16x16').output_shape[1:3]]
    ...
    model_checkpoint = ModelCheckpoint(filepath='./checkpoint/new_anchor_256_fix_DBset_2_epoch-{epoch:02d}_loss-{loss:.4f}.h5',
                                    monitor='val_loss',
                                    verbose=1,
                                    save_best_only=True,
    ...
    log_dir = './logs/scalars/'+ 'new_anchor_256_fix_DBset_2_'+datetime.now().strftime("%Y%m%d-%H%M%S")
